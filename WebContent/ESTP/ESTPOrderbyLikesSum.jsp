...
<%
	...
	Connection conn = contentsDao.getConn();
 	List<Contents> lst = contentsDao.getMBTIAllListOrderbyLikeSum(conn, mbti);
...
<form id="frm" action="<%=request.getContextPath() %>/ESTP/ESTPcommentProc2.jsp" method="post">
<div id="timeLine">
<div class="orderbyArea">
	<div class="orderbyHistory" style="background:#fff"><a href="<%=request.getContextPath() %>/index.jsp?currentPage=ESTP">최신순</a></div>
	<div class="orderbyClicked" id="orderbyClicked"><a href="<%=request.getContextPath() %>/index.jsp?currentPage=ESTPOrderbyLikesSum">공감순</div>
	<div class="allPostBtn"><a href="<%=request.getContextPath() %>/ESTP/ESTPContentsForm2.jsp">글쓰기</a></div>
</div>
<%
...
%>
	<div class="reading">
		...
		<div class="likeArea">
			...
			<div class="likeBtn" >
				<a href="<%=request.getContextPath() %>/ESTP/ESTPLikesProc2.jsp?likeBtn<%=count4++ %>=<%=t.getContentsid() %>" style="color:black;">공감하기</a>
			</div>		
